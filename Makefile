...
DETECTION_IMG ?= image-detection-controller:latest
CREATION_IMG ?= resource-creation-controller:latest
...
.PHONY: build
build: build-detection build-creation ## Build both controller binaries.

.PHONY: build-detection
build-detection: manifests generate fmt vet ## Build image-detection-controller binary.
	go build -o bin/image-detection-controller cmd/detection/main.go

.PHONY: build-creation
build-creation: manifests generate fmt vet ## Build resource-creation-controller binary.
	go build -o bin/resource-creation-controller cmd/creation/main.go

.PHONY: run-detection
run-detection: manifests generate fmt vet ## Run image detection controller from your host.
	go run ./cmd/detection/main.go

.PHONY: run-creation
run-creation: manifests generate fmt vet ## Run resource creation controller from your host.
...
.PHONY: docker-build
docker-build: docker-build-detection docker-build-creation ## Build both docker images.

.PHONY: docker-build-detection
docker-build-detection: ## Build docker image for detection controller.
	$(CONTAINER_TOOL) build -t ${DETECTION_IMG} -f Dockerfile.detection .

.PHONY: docker-build-creation
docker-build-creation: ## Build docker image for creation controller.
	$(CONTAINER_TOOL) build -t ${CREATION_IMG} -f Dockerfile.creation .

.PHONY: docker-push
docker-push: docker-push-detection docker-push-creation ## Push both docker images.

.PHONY: docker-push-detection
docker-push-detection: ## Push detection controller docker image.
	$(CONTAINER_TOOL) push ${DETECTION_IMG}

.PHONY: docker-push-creation
docker-push-creation: ## Push creation controller docker image.
	$(CONTAINER_TOOL) push ${CREATION_IMG}
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name image-resource-controller-builder
	$(CONTAINER_TOOL) buildx use image-resource-controller-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm image-resource-controller-builder
	rm Dockerfile.cross
...
# gomodver extracts the version of a module from go.mod
# $1 - module path (e.g. sigs.k8s.io/controller-runtime)
gomodver = $(shell grep '$(1)' go.mod | head -1 | awk '{print $$2}')
