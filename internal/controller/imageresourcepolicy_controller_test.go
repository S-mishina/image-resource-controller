...
var _ = Describe("ImageResourcePolicy Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ImageResourcePolicy")
			err := k8sClient.Get(ctx, typeNamespacedName, imageresourcepolicy)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ImageResourcePolicy")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ImageResourcePolicyReconciler{
				Client: k8sClient,
